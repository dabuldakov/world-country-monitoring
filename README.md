...
Приложение слушает контейнерный порт `8080` и публикуется на хосте как `8087`
(для мониторинга/нагрузочных тестов); фронтенд ходит в API через nginx по адресу
`http://backend:8080`. Health-эндпоинт Actuator: `http://<host>:8087/actuator/health`.
