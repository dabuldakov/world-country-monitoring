...
class WorldBankClient(
    ...
    fun getAllHistoryGDPPerCapitaByCountry(countryCode: String): WorldBankModel? {
        return try {
            restTemplate.getForObject(buildGDPPerCapitaUrl(countryCode), WorldBankModel::class.java)
        } catch (e: Exception) {
            logger.error("Error while getAllHistoryGDPPerCapitaByCountry $countryCode", e)
            null
        }
    }
    ...
    fun getAllHistoryDebtAmountByCountry(countryCode: String): WorldBankModel? {
        return try {
            restTemplate.getForObject(buildDebtAmountUrl(countryCode), WorldBankModel::class.java)
        } catch (e: Exception) {
            logger.error("Error while getAllHistoryDebtAmountByCountry $countryCode", e)
            null
        }
    }
    ...
    fun getAllHistoryPopulationByCountry(countryCode: String): WorldBankModel? {
        return try {
            restTemplate.getForObject(buildPopulationUrl(countryCode), WorldBankModel::class.java)
        } catch (e: Exception) {
            logger.error("Error while getAllHistoryPopulationByCountry $countryCode", e)
            null
        }
    }
    ...
    private fun buildGDPPerCapitaUrl(countryCode: String): String {
        val queryParams = StringBuilder()
        queryParams.append("${Constant.DATABASE_ID}=${URLEncoder.encode(Constant.DATABASE_WB, "UTF-8")}&")
        queryParams.append("${Constant.INDICATOR}=${URLEncoder.encode(Constant.INDICATOR_GDP_PER_CAPITA, "UTF-8")}&")
        queryParams.append("${Constant.REF_AREA}=${URLEncoder.encode(countryCode, "UTF-8")}&")
        queryParams.append("${Constant.skip}=0")
    ...
    private fun buildDebtAmountUrl(countryCode: String): String {
        val queryParams = StringBuilder()
        queryParams.append("${Constant.DATABASE_ID}=${URLEncoder.encode(Constant.DATABASE_WB, "UTF-8")}&")
        queryParams.append("${Constant.INDICATOR}=${URLEncoder.encode(Constant.INDICATOR_DEBT_AMOUNT, "UTF-8")}&")
        queryParams.append("${Constant.REF_AREA}=${URLEncoder.encode(countryCode, "UTF-8")}&")
        queryParams.append("${Constant.skip}=0")
    ...
    private fun buildPopulationUrl(countryCode: String): String {
        val queryParams = StringBuilder()
        queryParams.append("${Constant.DATABASE_ID}=${URLEncoder.encode(Constant.DATABASE_WB, "UTF-8")}&")
